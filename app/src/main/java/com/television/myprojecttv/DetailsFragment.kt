...
import com.mygallery.mytelevisionapp.PlaybackActivity
...
//        buildUI()
   ...
   /*
    ...
    /*private fun buildUI() {
        var seletor:ClassPresenterSelector = ClassPresenterSelector()
        seletor.addClassPresenter(DetailsOverviewRow::class.java,
            FullWidthDetailsOverviewRowPresenter(DetailsDescriptionPresenter()))
        var madapter:ArrayObjectAdapter = ArrayObjectAdapter(seletor)
        var detailsOverviewRow:DetailsOverviewRow = DetailsOverviewRow("detailsMovie Media")
        detailsOverviewRow.setImageDrawable(activity?.resources?.getDrawable(R.drawable.ic_launcher_foreground))

        var saoa:SparseArrayObjectAdapter = SparseArrayObjectAdapter()
        saoa.set(ACTION_PLAY.toInt(),Action( 1, "Buy $0.99"))
        saoa.set(ACTION_REWIND.toInt(), Action(2, "Buy $1.99"))
        saoa.set(ACTION_REWIND.toInt(), Action(3, "Buy $2.99"))
        detailsOverviewRow.actionsAdapter = saoa
